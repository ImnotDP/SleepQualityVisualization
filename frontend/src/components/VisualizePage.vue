  ...
  <div class="viz-page">
    ...
    <div class="viz-section" :class="{ 'viz-fullscreen': fsSection === 'histogram' }">
      <div class="section-header">
        ...
        <span class="section-fs-btn" @click.stop="toggleFs('histogram')" :title="fsSection==='histogram'?'退出全屏':'全屏查看'">{{ fsSection==='histogram'?'✕':'⛶'}}</span>
      </div>
      <div class="section-body">
        <div class="chart-card">
...
function onModelChange(key) {
  ...
  // Update R² chart highlight
  if (mc) buildModelCharts(mc, modelKey)
...
function buildModelCharts(mc, highlightKey) {
  if (!mc) return
  ...
  const colors = r2vals.map((v, i) => {
    const k = entries[i].key
    if (k === highlightKey) return '#67c23a'
...
onMounted(async () => {
  ...
  try {
    ...
    cachedFeatureData.value = fa.data
    const mc = fa.data?.model_comparison
    if (mc) {
      perModelFI.value = fa.data?.per_model_fi || {}
      buildModelCharts(mc, fa.data?.best_model)
      // 默认展示最佳模型特征重要性
      onModelChange('')
    }
