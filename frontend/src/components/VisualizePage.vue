...
import { getTrendData, getScatterData, getHistogramData, getCorrelationData, getStagePieData, getEnvironmentData, getEnvironmentVsQuality } from '../api/sleep'
...
const ALL_SECTIONS = ['trend', 'scatter', 'histogram', 'correlation', 'stage', 'environment']
const activeSections = ref([...ALL_SECTIONS])
const isMobile = ref(false)

function checkMobile() {
  isMobile.value = window.innerWidth < 768
  activeSections.value = isMobile.value ? ['trend'] : [...ALL_SECTIONS]
}
let resizeTimer = null
function onResize() { clearTimeout(resizeTimer); resizeTimer = setTimeout(checkMobile, 200) }
onMounted(() => { checkMobile(); window.addEventListener('resize', onResize) })
...
.viz-collapse { --el-collapse-header-bg-color: #1a2a3a; --el-collapse-content-bg-color: #0f1923; border: none; }
.viz-collapse :deep(.el-collapse-item) { margin-bottom: 10px; border: 1px solid #2a3a4a; border-radius: 12px; overflow: hidden; background: #1a2a3a; transition: all 0.3s ease; }
.viz-collapse :deep(.el-collapse-item:hover) { border-color: #3a5a7a; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }
.viz-collapse :deep(.el-collapse-item__header) { height: auto !important; padding: 14px 20px; background: linear-gradient(135deg, #1e3044 0%, #1a2a3a 100%); border-bottom: 1px solid #2a3a4a; font-size: 15px; font-weight: 600; color: #e0e0e0; line-height: 1.4; }
.viz-collapse :deep(.el-collapse-item__wrap) { background: #0f1923; border: none; }
.viz-collapse :deep(.el-collapse-item__content) { padding: 20px; }
.viz-collapse :deep(.el-collapse-item__arrow) { display: none; }

.section-header { display: flex; align-items: center; gap: 10px; width: 100%; }
